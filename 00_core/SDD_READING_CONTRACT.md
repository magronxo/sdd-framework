...
3) `00_core/AGENT_DECISION_TABLE.md` (only if classification/triage is needed)
4) Feature-local artifacts (only what you need for the current phase):
   ...
   - design: `artifacts/design/<feature_id>.md`
...
Read `00_core/SDD_GUIDE.md` only if:
...
Read `90_transitional/*` (if present) only if a user explicitly requests historical context.
