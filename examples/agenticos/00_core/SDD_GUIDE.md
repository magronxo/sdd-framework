# SDD Guide – AgenticOS (Simplificat)
...
4. **hardware_aware** – Tota decisió ha de passar el filtre de recursos (Orange Pi).
5. **no_direct_mutation** – Mai es modifica codi directament; sempre a través de documents de feature i specs.
6. **external_dev_first** – Els problemes de flux, governança, context i integració agentica s'han de resoldre fora del Kernel abans d'obrir canvis a Ring 0.
...
## Capa Externa de Desenvolupament del Kernel

Abans de tocar runtime, existeix una capa externa de desenvolupament definida a:

- `03_operations/ROADMAP.md`
- `02_policies/FRAMEWORK_INTEGRATION_MAP.md`

La seva funció és ordenar:

- governança de `AGENTS.md`
- ús del `context-engine`
- taxonomia de skills
- decisió `prompt vs skill`
- auditories externes
- relació amb frameworks com `gentle-ai` o `.opencode`

**Regla:** Si el problema és de procés extern, no s'arregla dins del Kernel.

### Implementació: SDT vs TDD

**Per components deterministes (Go, C, etc.):** Usem **TDD** (Test-Driven Development) durant la fase d'implementació.
- Escriure test `_test.go` → Implementar codi mínim → Refactoritzar
...
**Fluxe híbrid pel Kernel:**
```
SDD (Documents) → TDD (Codi Go) → Validació SDT (Sistema complet) → Auditoria → Archive
```

| Estat | Rol | Prompt/Skill | Artifact | Descripció |
|-------|-----|--------------|----------|------------|
| **DESIGN** | Designer | `01_execution/prompts/designer.md` | `00_project_documentation/SDD/artifacts/design/<feature_id>.md` | Defineix el QUÈ: arquitectura, components, hardware budget. |
| **SPEC** | Specifier | `01_execution/prompts/specifier.md` | `00_project_documentation/SDD/artifacts/specs/<feature_id>.md` | Defineix el COM: inputs, outputs, errors, SDT scenarios, Gherkin. |
...
| **TASKS** | Planner | `01_execution/prompts/planner.md` | `00_project_documentation/SDD/artifacts/tasks/<feature_id>.md` | Genera una llista de tasques mínima i ordenada a partir d'una spec validada. |
...
| **AUDIT** | Auditor | `01_execution/skills/sdd-audit.md` | `audit_report` | Auditoria lleugera: spec-codi, tests, qualitat. |
...
- **Model:** GPT-5-mini (ràpid, barat)
- **Abast:** Spec-codi coherence, tests, qualitat bàsica
- **Output:** Informe a `audit_reports/audit_[feature]_[data].md`
- **Resultat:** PASS/WARN/FAIL
- **Acció:** Si FAIL → deep audit; Si WARN/PASS → Archive

**sdd-deep-audit (Profunda):**
- **Trigger:** Manual (`/audit-deep`) o cada 5 features
- **Model:** GPT-5 (exhaustiu)
- **Abast:** Seguretat, arquitectura, consistència global
- **Output:** Informe a `audit_reports/audit_batch_[n]_[data].md`
...
**Externa (ara):**
- Ring 0 (Kernel) audita via skills externes (OpenCode)
- Impossible auditar-se a si mateix (immutable, conflicte interessos)
- Més segur, més ràpid, més flexible

**Interna (futur):**
- Ring 1+ (Departaments) poden tenir auditor intern (`05_auditor`)
- Dashboard IDE permetrà desenvolupar Ring 1+ des de dins
- Sistema autònom per a departaments, NO per al Kernel

**Regla:** Auditoria del Kernel sempre EXTERNA. Auditoria de Departaments pot ser INTERNA (Fase 2+).
...
3. opcionalment es contrasta amb `gentle-ai`
...
- `gentle-ai`: complement d'auditoria, memòria externa i revisió de specs
- `.opencode`: entorn/harness extern compatible
...
7. ARCHIVE:  Sync a specs main, update features_for_specs.
             Executar `.\04_tools\context.ps1 index` per actualitzar l'armeria.
             ↓
```
...
|---------|-----------|--------|
| `00_project_documentation/SDD/artifacts/design/` | Documents de disseny (QUÈ) | `feat-XXX_nom.md` |
| `00_project_documentation/SDD/artifacts/specs/` | Especificacions (COM) | `feat-XXX_nom.md` |
| `00_project_documentation/SDD/artifacts/tasks/` | Desglossament de tasques | `feat-XXX_nom.md` |
| `00_project_documentation/SDD/artifacts/features_for_specs/` | JSON d'estat | `feat-XXX.json` |
...
### Document de disseny (`00_project_documentation/SDD/artifacts/design/<feature_id>.md`)

Segueix la plantilla `00_project_documentation/SDD/templates/design.md` i inclou:

- Motivació i components afectats
- Data models (Go structs o JSON schemas)
- Diagrama Mermaid del flux
- Hardware budget (RAM, CPU, disc)
- Preguntes obertes `[?]` (han de ser ZERO per passar a SPEC)

### Especificació funcional (`00_project_documentation/SDD/artifacts/specs/<feature_id>.md`)

Segueix la plantilla `00_project_documentation/SDD/templates/specs.md` i inclou:
...
Segueix el format definit a `00_project_documentation/SDD/00_core/SDD_FEATURE_FORMAT.md`.

---

## Procés pas a pas

1. **Crear feature record**: Crea `00_project_documentation/SDD/artifacts/features_for_specs/<feature_id>.json` amb `state: DESIGN`
2. **Executar Designer**: Llegeix `01_execution/prompts/designer.md`, crea `00_project_documentation/SDD/artifacts/design/<feature_id>.md`, actualitza a `state: SPEC`
3. **Executar Specifier**: Llegeix `01_execution/prompts/specifier.md`, crea `00_project_documentation/SDD/artifacts/specs/<feature_id>.md`, actualitza a `state: VALIDATION`
...
5. **Executar Planner**: Llegeix `01_execution/prompts/planner.md`, crea `00_project_documentation/SDD/artifacts/tasks/<feature_id>.md`, actualitza a `state: IMPLEMENT`
...
8. **Executar Auditor + Archive**: Genera report a `00_project_documentation/SDD/audit_reports/` i tanca la feature a `state: ARCHIVE`
...
**Què cal actualitzar:**
1. **Document de disseny** (`01_design/*.md`): Afegir secció "Estat d'Implementació" amb:
...
2. **Project Map** (`00_project_documentation/03_PROJECT_MAP.md`): Actualitzar:
...
3. **Altres documents** (si cal):
   - `01_MANIFEST.md` (si hi ha canvis filosòfics)
   - `04_PARKING_LOT.md` (si s'eliminen features pendents)
...
**Actualitzat:** 2026-03-29 - Afegida regla d'actualització de documents.
