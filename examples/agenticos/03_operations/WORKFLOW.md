...
Provide the operational flow for meaningful work in AgenticOS external Kernel development.
...
does it touch Kernel runtime or only external development?
is the task open, closed, or ambiguous?

If the work is still an idea/seed (not yet an SDD feature), use PRE-SDD:
...
use context-engine first for semantic discovery when the task is large or unclear
...
do not touch Kernel runtime unless the task explicitly requires it
Phase 5 — Validation
